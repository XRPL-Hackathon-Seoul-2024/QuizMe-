    ...
    <div>
      ...
      <div className="flex flex-col gap-4 items-center justify-center h-full bg-background overflow-auto px-4 w-[30rem] max-w-md mx-auto">
        {problems.map((curProb, index) => {
          return <ProblemComponent key={index} idx={index} props={curProb} />;
        })}
      </div>
      <div className="flex w-full justify-center w-[30rem] max-w-md mb-20">
        <button
          onClick={() => {
            writeContract({
          ...
          className="w-24 h-12 bg-black text-white rounded-3xl font-bold"
        >
          Submit
        </button>
      </div>
...
function ProblemComponent({ idx, props }: { idx: number; props: any }) {
              ...
              {options?.map((option: any) => {
                return (
                  <Button
                    ...
                    onClick={() => {}}
                    className="w-full justify-start border border-gray-400 border-2 h-12"
                  >
                    {option.title}
                  </Button>
                );
